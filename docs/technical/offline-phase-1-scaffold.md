...
批准的[离线只读分析切片](offline-read-only-analysis.md)，不得扩大为公开 API。
...
另行批准的 `/offline/` 读取路径见
[离线只读分析设计](offline-read-only-analysis.md)。其他路径返回稳定 `NOT_FOUND`，
非 GET 请求返回 `METHOD_NOT_ALLOWED`。错误不包含堆栈、请求体、绑定值或第三方响应。
这些接口都不是后续 Web/MCP 业务契约。
