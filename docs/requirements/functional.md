...
- 候选阶段：Phase 1–2
- 依赖：`DQ-06`、`DQ-08`
- 要求：API、MCP 和 Web 运营平台 MUST 显示同步时间、错误、覆盖截止时间和稳定状态。
...
- 候选阶段：Phase 3–4
- 依赖：`DQ-07`
- 要求：Web MUST 支持首期素材、广告创建与管理、数据分析、测试与优化、连接、账户
  绑定、同步状态、审批、审计和 Emergency stop。
- 验收：三个核心场景可跨模块完成；网页不显示 Token、不直连 Meta API，用户只能
  看到授权 Workspace。
