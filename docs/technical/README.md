...
助手，需求映射和六项旧 ADR 已完成一轮评审；但 G0 外部事实、ADR-003、ADR-005 以及
各技术文档的完整替代方案仍未确认。因此本目录除已接受 ADR 明确约束的边界外，继续只
供后续比较，不得表述为整体选型、部署或验证。
...
- `DG0` 通过不自动接受技术方案；已接受 ADR 只约束其明确范围，其余替代方案、安全
  落点和外部事实仍须逐项评审。
...
离线独立会话前向评测已完成五类无答案 Case：最终 5/5 均为 8/8，两项首次拒绝经 Skill
