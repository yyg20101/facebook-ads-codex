...
负责人随后允许离线 Codex 独立会话前向评测。五个无黄金答案测试包、准备器、操作者
声明和确定性评分器先证明材料与接收链路可重复；随后五个 Case 的最终结果均为 8/8。
两项首次拒绝在收紧 Skill 后用全新会话重测。该结果只覆盖固定 fixture，且隔离协议未
被技术独立验证；它不证明真实账户结论、用户价值、通用模型质量或 G2/G3 已满足。
...
2026-08-09，独立会话前向评测先完成准备链路验证，随后实际运行 5 个首次会话和 2 个
失败修正后的全新重测会话。`001`、`002`、`004` 首次 8/8；`003`、`005` 首次因否定式
禁用词被拒绝，收紧 Skill 后重测 8/8。最终状态为 5/5 `PASS`；操作者声明仍为
`attestation_independently_verified: false`，该记录不是用户价值、真实账户、通用模型
或 Gate 证据，G0 继续为 `PARTIAL`。
